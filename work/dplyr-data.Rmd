...
facebook2 <- facebook %>% mutate(friend_ratio = friendships_initiated / friend_count)
# 2. Find the median `friend_ratio` for males and females.
facebook2 %>% group_by(gender) %>%
          summarise(fr_median = median(friend_ratio, na.rm = TRUE))

# 1 & 2 in one fell swoop
facebook %>% mutate(friend_ratio = friendships_initiated / friend_count) %>%
         group_by(gender) %>%
         summarise(fr_median = median(friend_ratio, na.rm = TRUE))
```
...
# substitute( ) | Programming with dplyr

## substitute( )

```{r, collapse=FALSE, cache=TRUE}
friend_ratio <- function(data, nom, denom) {
  call <- substitute(data_new <- data %>%
                       transmute(friend_ratio = nom / denom),
                     list(nom = as.name(nom),
                          denom = as.name(denom)))
  eval(call)
  return(as.vector(data_new$friend_ratio))
}

facebook$friend_ratio <- friend_ratio(data = facebook,
                                      nom = "friendships_initiated",
                                      denom = "friend_count")
facebook %>% tbl_df %>% select(userid, age, gender, friend_count) %>% arrange(desc(friend_count))
```
...
```{r, collapse=FALSE, cache=TRUE}
group_median <- function(data, id_vars, var, na.rm) {
  id_vars_sym <- lapply(id_vars, as.symbol)
  call <- substitute(data_new <- data %>%
                       group_by_(.dots = id_vars) %>%
                       summarise(var_median = median(var, na.rm = na.rm)),
                     list(id_vars = id_vars_sym,
                          var = as.name(var)))
  eval(call)
  return(data_new)
}
facebook %>% mutate(minor = ifelse(age < 18, TRUE, FALSE)) %>%
                    group_median(id_vars = c("gender", "minor"),
                          var = "friend_ratio", na.rm = TRUE)
```
...
## Hands-on script 6-dplyr-data.R {.flexbox .vcenter}
