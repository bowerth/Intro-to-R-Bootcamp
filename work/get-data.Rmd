...
- Use `Export (SDMX)`: `SNA_TABLE1/AUS+AUT+BEL.B1_GA+B1G_P119+B1G.C+V`
- modify URL in SDMX browser at [sdmx.rdata.work](http://sdmx.rdata.work/OECD/SNA_TABLE1.AUS+AUT+BEL.B1_GA+B1G_P119+B1G.C+V/)
- download file from [sdmx.rdata.work](http://sdmx.rdata.work/getdownloadsdmx?) using `getURL()`

```{r, collapse=TRUE, cache=TRUE, eval=FALSE}
tt <- getURL("http://sdmx.rdata.work/OECD/SNA_TABLE1.AUS+AUT+BEL.B1_GA+B1G_P119+B1G.C+V/?start=2009")
read.csv(text = tt)
```
...
- The [`nsoApi`](https://github.com/bowerth/nsoAPI) package provides functions to harmonize information into an R standard time series format
...
read.csv("../data/cbs_data.csv")[1:5, 1:4]
```
